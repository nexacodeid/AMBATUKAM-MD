...
const models = [
  'anime2d', 'anime3d', 'ghibli', 'pixel', 'cyberpunk',
  'sketch', 'painting', 'comic', 'cartoon', 'oilpaint',
  'watercolor', 'vintage', 'dreamlike', 'fantasy'
]
...
async function filterXterm(imageurl, model, m) {
  ...
  if (!ai.status) {
    throw ai.message || `API Error: ${JSON.stringify(ai)}`
  }
...
async function uploadImage(buffer) {
  ...
  if (!json || !json[0] || !json[0].src) {
    throw `Telegraph upload failed: ${JSON.stringify(json)}`
  }
