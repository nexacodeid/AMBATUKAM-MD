...
async function handler(m, { conn, args }) {
    ...
    if (m.sender in confirm) m.reply(`Kamu sudah melakukan permintaan code event, silahkan kirim code event yang kamu miliki`)
    try {
...
handler.before = async m => {
   ...
   try {
    if (/^Xyazellackermanxyz$/i.test(txt)) {
        if (user.lastcode >= 1) {
        let uang = Math.floor(Math.random() * 80000000)
        let exp = Math.floor(Math.random() * 1000000)
        let cash = 500 * 1
        usec = user.lastcode * 1
          user.money += uang * 1
          user.exp += exp * 1
          user.cash += cash * 1
          user.lastcode = 0
...
_Jangan Lupa bersyukur dan berterima kasih yaa :)_`)
      ...
      clearTimeout(timeout)
       ...
       m.reply(`🚫 *kamu sudah menggunakan code tersebut*`)
        ...
        clearTimeout(timeout)
        delete confirm[m.sender]
        if (usec > (user.lastcode * 1)) user.lastcode = usec * 1
