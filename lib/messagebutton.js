...
class Button extends BaseBuilder {
	...
	async send(jid, { ...options } = {}) {
		if (this._buttons.length < 1) throw new Error('Button requires at least one button');
...
class Carousel extends BaseBuilder {
	...
	async send(jid, { ...options } = {}) {
		if (this._cards.length < 1) throw new Error('Carousel requires at least one card');
