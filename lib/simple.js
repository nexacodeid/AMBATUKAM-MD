...
import { rememberJidMappings, resolvePrivateSendJid } from './jid-routing.js';
...
export function makeWASocket(connectionOptions, options = {}) {
	...
	const OrigProfilePictureUrl = conn.profilePictureUrl.bind(conn);
	let sock = Object.defineProperties(conn, {
		...
		sendMessage: {
			async value(jid, content, options = {}) {
				const targetJid = await resolvePrivateSendJid(conn, jid, options?.quoted?.key);
				return OrigMsg(
					targetJid,
					{
						...content,
						...
						annotations: [
							{
								embeddedContent: {
									embeddedMusic: {
										...
										artistAttribution: 'https://lynk.id/ellz',
									},
								},
								embeddedAction: true,
							},
		...
		relayMessage: {
			async value(jid, message, options = {}) {
				const targetJid = await resolvePrivateSendJid(conn, jid);
				return OrigRelay(targetJid, message, options);
			},
		},
		decodeJid: {
			value(jid) {
		...
		profilePictureUrl: {
			async value(jid, type = 'image', query = 'url') {
				try {
					const timeoutMs = typeof query === 'number' ? query : 15000;
					const url = await OrigProfilePictureUrl(jid, type, timeoutMs);
		...
		getFile: {
			...
			async value(PATH, saveToFile = false) {
				...
				if (data && saveToFile && !filename) ((filename = path.join(__dirname, '../tmp/' + Date.now() + '.' + type.ext)), await fs.promises.writeFile(filename, data));
				const result = {
					res,
					filename,
					...type,
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				if (/webp/.test(type.mime) || (/image/.test(type.mime) && options.asSticker)) mtype = 'sticker';
					...
					mtype = 'audio';
					const audioExt = String(type.ext || '').replace(/^\./, '').toLowerCase();

					if (ptt) {
						convert = await toPTT(file, audioExt);
						file = convert.data;
						pathFile = convert.filename;
						mimetype = options.mimetype || 'audio/ogg; codecs=opus';
					} else if (type.mime === 'audio/mpeg' || audioExt === 'mp3') {
						// MP3 dari downloader sudah kompatibel dengan WhatsApp.
						// Mengonversinya lagi hanya membuang waktu dan sebelumnya bisa
						// membuat path input/output FFmpeg menjadi sama.
						mimetype = options.mimetype || 'audio/mpeg';
					} else {
						convert = await toAudio(file, audioExt);
						file = convert.data;
						pathFile = convert.filename;
						mimetype = options.mimetype || 'audio/mpeg';
					}
    ...
    addButton(text, id = text.toLowerCase()) {
    ...
    async send(jid, quoted) {
        return this.conn.sendButton(jid, this.data, { quoted })
    }
},  // koma di sini karena masih ada property lain di bawahnya
		sendButton: {
			async value(jid, content = {}, options = {}) {
				let header = {};
...
export function smsg(conn, m) {
	...
	m = M.create(m);
	void rememberJidMappings(conn, m.key).catch((error) => {
		conn.logger?.warn?.(`Gagal menyimpan mapping PN/LID: ${error.message || error}`);
	});
