...
const resize = async (image, width, height) => {
    const input = await fetchBuffer(image)
...
async function safeAdReply(conn, jid, text, title = '', body = '', thumbnail = false, sourceUrl = false, quoted = null) {
    let img = false

    try {
        if (thumbnail) {
            img = Buffer.isBuffer(thumbnail) ? thumbnail : await resize(thumbnail, 300, 175)
        }

        if (!Buffer.isBuffer(img)) throw new Error('Thumbnail bukan Buffer')

        // Coba kirim sebagai adReply terlebih dahulu.
        return await conn.adReply(jid, text, title, body, img, sourceUrl, quoted)
    } catch (err) {
        console.error('[werewolf] adReply gagal, fallback ke image message:', err?.message || err)

        // Kalau adReply di simple.js error "Result is not a buffer", tetap kirim gambar biasa.
        try {
            if (!Buffer.isBuffer(img) && thumbnail) {
                img = Buffer.isBuffer(thumbnail) ? thumbnail : await resize(thumbnail, 300, 175)
            }
            ...
            console.error('[werewolf] fallback image gagal, fallback ke text:', err2?.message || err2)
        }

        if (conn.reply) return await conn.reply(jid, text, quoted || false)
...
async function pagi(conn, x, data) {
    ...
    return await safeAdReply(conn, x.room, pagii(x), 'W E R E W O L F', '', thumb1, false)
}
...
async function voting(conn, x, data) {
    ...
    return await safeAdReply(conn, x.room, textnya, 'W E R E W O L F', '', thumb2, false)
}
...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        return safeAdReply(conn, x.room, textnya, 'W E R E W O L F', '', thumb3, false).then(() => {
            changeDay(x.room, data)
        ...
        if (hasil_vote.role === "werewolf") {
            ...
            return await safeAdReply(conn, x.room, textnya, 'W E R E W O L F', '', thumb4, false).then(() => {
                changeDay(x.room, data)
                voteDone(x.room, data)
                resetVote(x.room, data)
                clearAllVote(x.room, data)
...
async function win(x, t, conn, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await safeAdReply(conn, sesinya, textnya, 'W E R E W O L F', '', thumb5, false).then(() => {
            delete data[x.room]
        })
        ...
        return await safeAdReply(conn, sesinya, textnya, 'W E R E W O L F', '', thumb6, false).then(() => {
            delete data[x.room]
        })
    }
}
